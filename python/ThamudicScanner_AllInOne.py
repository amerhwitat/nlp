#!/usr/bin/env python3
"""Self-contained all-in-one ancient-language/NLP scanner."""
...
def run_worker(args,timeout):
    ...
    return r
def ocr(path,langs=None):return run_worker(["--worker-ocr",str(path),"--languages",",".join(langs or ["en","ar"])],max(30,int(os.environ.get("THAMUDIC_OCR_TIMEOUT","180"))))
def pdf_page(path,page,out):return run_worker(["--worker-pdf",str(path),"--page",str(page),"--output",str(out)],max(30,int(os.environ.get("THAMUDIC_PDF_TIMEOUT","120"))))
...
if __name__=="__main__":
    try:raise SystemExit(cli(sys.argv[1:],"Thamudic / Ancient North Arabian"))
