...
This is a web-native adapter over the existing Python scanner modules. It stores
source text, script detection, transliteration, evidence-backed translation,
media provenance and analysis metadata in ArtifactDatabase without changing the
scanner's evidence-first behavior.
"""
...
def artifact_analyze(request: dict[str, Any]):
    text = str(request.get("text", "")); script = str(request.get("script", "Dadanitic")); target = str(request.get("target_language", "en"))
    source_language = str(request.get("source_language", script))
    if not text.strip():
    ...
    return _ingest({
        "title": str(request.get("title", "NLP/Thamudic analysis")), "source": str(request.get("source", "web")),
        "media_type": "text", "original_text": text, "source_language": source_language,
        "script_variant": script, "target_language": target, "scan": {**scan, "language_scan": language_scan},
        "translation": translation, "source_form": "script", "metadata": {"pipeline": "web-artifact-analyze", "language_scan": language_scan},
    })
...
def artifact_analyze_ancient(request: dict[str, Any]):
    ...
    if not text.strip() or not source_language.strip() or not target.strip():
    ...
    return _ingest({
        "title": str(request.get("title", "Ancient language artifact")), "source": str(request.get("source", "web")),
        "media_type": "text", "original_text": text, "source_language": source_language,
        "script_variant": source_language, "target_language": target, "scan": {"matched": bool(scan.get("matches")), "language": source_language, "script_variant": source_language, "confidence": 0, "codepoints": [], "language_scan": scan},
        "translation": translation, "source_form": source_form, "metadata": {"pipeline": "web-artifact-ancient", "language_scan": scan},
    })
...
async def artifact_analyze_upload(file: UploadFile = File(...), script: str = "Dadanitic", target_language: str = "en"):
    ...
    if len(data) > int(os.getenv("THAMUDIC_MAX_UPLOAD_BYTES", str(10 * 1024 * 1024))):
        raise HTTPException(status_code=413, detail="Uploaded artifact exceeds configured size limit")
    ...
    with tempfile.TemporaryDirectory(prefix="thamudic-artifact-") as tmp:
        ...
        try:
            extracted = extract_media_text(path)
        except Exception as exc:
            raise HTTPException(status_code=422, detail=f"Media extraction failed: {exc}") from exc
    text = str(extracted.get("text", ""))
    if not text.strip():
        raise HTTPException(status_code=422, detail="No text could be extracted from artifact")
    scan = scan_text(text, [])
    language_scan = scan_source_language_text(text, language=script)
    translation = translate_text(text, script=script, target_language=target_language)
    return _ingest({
        "title": Path(file.filename or "artifact").name, "source": "web-upload", "media_type": suffix.lstrip("."),
        "original_text": text, "source_language": script, "script_variant": script, "target_language": target_language,
        "scan": {**scan, "language_scan": language_scan}, "translation": translation,
        "media": {"media_path": file.filename, "media_type": suffix.lstrip("."), "sha256": digest, "extraction_method": extracted.get("method", "media_pipeline"), "extraction_status": extracted.get("status", "ok"), "metadata": extracted.get("metadata", {})},
        "metadata": {"pipeline": "web-artifact-upload", "original_filename": file.filename, "sha256": digest, "extracted": extracted},
    })
