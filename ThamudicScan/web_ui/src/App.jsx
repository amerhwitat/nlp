...
import {
  browserSpeak, exportScriptSummary, exportSession, getAlphabetLanguages, getScriptSummary, getSession,
  getVoiceCapabilities, pauseVoice, resumeVoice, scanFile, scanSourceLanguage, scanText,
  stopVoice, subscribeProgress, translateAncient, translateText, validateText, voiceSpeak,
} from './api'

const EXAMPLE = '𐪀𐪁𐪂 𐪃𐪄'
const SOURCE_LANGUAGES = [['', 'Auto detect']]
...
export default function App() {
  ...
  const [sessionId, setSessionId] = useState(''); const [progress, setProgress] = useState(null)
  const [summary, setSummary] = useState(null); const [scriptSummary, setScriptSummary] = useState(null)
  const [results, setResults] = useState([]); const [validation, setValidation] = useState(null)
  const [languages, setLanguages] = useState([]); const [voice, setVoice] = useState(null)
  const [busy, setBusy] = useState(false); const [translationBusy, setTranslationBusy] = useState(false)
  const [sourceScanBusy, setSourceScanBusy] = useState(false); const [summaryBusy, setSummaryBusy] = useState(false); const [error, setError] = useState('')
  const keywordList = useMemo(() => keywords.split(',').map((value) => value.trim()).filter(Boolean), [keywords])

  useEffect(() => {
    getAlphabetLanguages().then((response) => setLanguages(response.languages || [])).catch(() => {})
    getVoiceCapabilities().then(setVoice).catch(() => {})
  }, [])
    ...
    <main>
      ...
      <section className="workspace-grid">
        ...
        <aside className="panel tools-panel">
          ...
          <label>Language
            <select value={sourceLanguage} onChange={(e) => setSourceLanguage(e.target.value)}>{SOURCE_LANGUAGES.concat(languages.map((id) => [id, id.replaceAll('-', ' ')] )).map(([value, label]) => <option key={`${value}-${label}`} value={value}>{label}</option>)}</select>
          </label>
          ...
          <div className="button-row"><button disabled={translationBusy || !text.trim()} onClick={handleAncientTranslate}>{translationBusy ? 'Translating…' : 'Translate selected language'}</button><button disabled={translationBusy || !text.trim()} onClick={handleTranslate}>ONA baseline</button></div>
