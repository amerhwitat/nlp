...
export async function exportScriptReport(payload, format = 'json') { const response = await fetch(`${API_BASE}/script-report/export`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...payload, format }) }); if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.detail || `Request failed (${response.status})`) }; const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `ancient-script-report.${format}`; link.click(); URL.revokeObjectURL(url) }
...
export function getTranslationLog() { return request('/translation-log') }
export function exportTranslationLog(format = 'json') { window.open(`${API_BASE}/translation-log/export?format=${encodeURIComponent(format)}`, '_blank', 'noopener,noreferrer') }
