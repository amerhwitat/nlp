...
static void PostProgress(HWND window, int value, const std::wstring& text) {
    auto* payload = new std::wstring(text);
    if (!PostMessageW(window, WM_ISOTOOL_PROGRESS, static_cast<WPARAM>(value), reinterpret_cast<LPARAM>(payload))) {
        delete payload;
    }
}
...
static bool SafeStep(HWND window, const std::wstring& name) {
    try {
        ...
        if (name == L"validate repository" && !ValidateRepository(window)) {
            return false;
        }
        Sleep(100);
        ...
        PostText(window, WM_ISOTOOL_LOG, L"[error] " + name + L" skipped after unknown runtime error");
        return false;
    }
}
...
static void RunPipeline(HWND window, bool buildIso) {
    ...
    for (size_t i = 0; i < steps.size(); ++i) {
        const bool ok = SafeStep(window, steps[i]);
        if (!ok) {
        ...
        if (steps[i].find(L"BIOS") != std::wstring::npos) {
            PostText(window, WM_ISOTOOL_LOG,
                     L"[boot] BIOS conventional first-stage address: 0x7C00; BIOS interrupts are only used in real mode.");
        }
        if (steps[i].find(L"UEFI") != std::wstring::npos) {
            PostText(window, WM_ISOTOOL_LOG,
                     L"[boot] UEFI entry: PE/COFF EFI application; firmware selects the load address; BIOS interrupts are not used.");
        }
        if (steps[i].find(L"fallback") != std::wstring::npos) {
            PostText(window, WM_ISOTOOL_LOG,
                     L"[boot] An unavailable or failed entry falls through to the next eligible boot-menu option.");
        }

        PostProgress(window,
                     static_cast<int>(((i + 1) * 100) / steps.size()),
...
static void SetBusy(bool busy) {
    EnableWindow(gAnalyze, !busy);
    EnableWindow(gImport, !busy);
    EnableWindow(gImages, !busy);
    EnableWindow(gBuild, !busy);
}
...
static void StartPipeline(HWND window, int commandId) {
    ...
    try {
        ...
        AppendLog(L"[error] Could not start worker thread: " + std::wstring(ex.what(), ex.what() + std::strlen(ex.what())));
        SetBusy(false);
    }
}
...
static LRESULT CALLBACK WndProc(HWND h, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        CreateWindowW(L"STATIC", L"ISO-Tool — GitHub / Local Source → Bootable ISO / IMG",
                      WS_CHILD | WS_VISIBLE, 20, 15, 850, 30, h, nullptr, nullptr, nullptr);
        gRepo = CreateWindowW(L"EDIT", L".", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
                              20, 50, 850, 28, h, nullptr, nullptr, nullptr);
        ...
        gBootStatus = CreateWindowW(L"STATIC", L"Boot validation: BIOS 0x7C00 | UEFI EFI entry | custom 0x8000 only when explicitly configured",
                                    WS_CHILD | WS_VISIBLE, 20, 122, 850, 24, h, nullptr, nullptr, nullptr);
        gLog = CreateWindowW(L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY,
                             20, 150, 850, 370, h, nullptr, nullptr, nullptr);
    ...
    case WM_COMMAND: {
        ...
        if (commandId == 1 || commandId == 2 || commandId == 4) {
            StartPipeline(h, commandId);
        } else if (commandId == 3) {
    ...
    case WM_ISOTOOL_PROGRESS: {
        ...
        if (payload) {
            SetWindowTextW(gStatus, payload->c_str());
            AppendLog(*payload);
            delete payload;
        }
        return 0;
    ...
    case WM_ISOTOOL_DONE:
        SetBusy(false);
        if (wParam == 0) {
            SetWindowTextW(gStatus, L"Pipeline completed");
            AppendLog(L"Pipeline reached the final step; inspect the log for build and boot decisions.");
        } else {
            SetWindowTextW(gStatus, L"Pipeline stopped");
            AppendLog(L"Pipeline stopped because a required validation step failed.");
        }
        return 0;
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    HWND window = CreateWindowW(windowClass.lpszClassName, L"ISO-Tool",
                                WS_OVERLAPPEDWINDOW, 100, 100, 920, 620,
