...
def _compile_systems(source:Path, output:Path, compiler:str, log):
    ...
    systems=detect_build_systems(source)
    ...
    for system in systems:
        ...
        if command is None:
            reports.append({'system':system,'status':'unavailable','command':None}); log(f'[build] {system}: toolchain unavailable; skipped')
            continue
        try:
            _run(command,source,log)
            if system == 'cmake':
                _run(['cmake','--build',str(build_root/system),'--config','Release','--parallel'],source,log)
            elif system == 'meson':
                _run(['meson','compile','-C',str(build_root/system)],source,log)
            reports.append({'system':system,'status':'built','command':command})
        except Exception as exc:
            reports.append({'system':system,'status':'failed','command':command,'error':str(exc)})
            log(f'[build] {system}: {exc}')
...
def build(source:Path,output:Path,compiler:str='auto',log=print)->dict:
    ...
    applications=discover_applications(source)
    (knowledge/'applications.json').write_text(json.dumps(applications,indent=2),encoding='utf-8')
    build_root,reports=_compile_systems(source,output,compiler,log)
    artifacts=_stage_tree(build_root,layout,log)
    manifest=layout['manifests']/f'build-result-{compiler}.json'
    manifest.write_text(json.dumps({'source':str(source),'output':str(output),'compiler':compiler,'build_tree':str(build_root),'build_systems':reports,'artifacts':artifacts,'plan':str(knowledge/'build-plan.json'),'ai_plan':str(knowledge/'ai-plan.json'),'applications':str(knowledge/'applications.json')},indent=2),encoding='utf-8'); log(f'[manifest] {manifest}')
    return {'layout':layout,'build_tree':build_root,'artifacts':artifacts,'manifest':manifest,'build_systems':reports}
