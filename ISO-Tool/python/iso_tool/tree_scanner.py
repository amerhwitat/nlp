...
@dataclass
class Entry:
    path:str; kind:str; size:int=0; sha256:str|None=None; language:str|None=None; children:list|None=None
...
def scan_tree(root:Path,manifest_path:Path|None=None)->dict:
    ...
    tree=walk(root)
    result={'root':str(root),'tree':asdict(tree),'summary':{'files':sum(counts.values()),'by_kind':counts,'languages':languages,'build_files':builds,'images':images,'scripts_requiring_review':scripts}}
    if manifest_path:
        Path(manifest_path).parent.mkdir(parents=True,exist_ok=True);Path(manifest_path).write_text(json.dumps(result,indent=2),encoding='utf-8')
    return result
