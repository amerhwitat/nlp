"""Cross-platform toolchain discovery with Windows environment and registry support."""
...
class Toolchain:
    family:str; role:str; name:str; path:str; version:str=''; source:str=''

def _version(exe:str)->str:
    for args in (('--version',),('/?',),('-version',)):
        try:
            p=subprocess.run([exe,*args],capture_output=True,text=True,timeout=5,errors='replace');text=(p.stdout or '')+'\n'+(p.stderr or '')
            line=next((x.strip() for x in text.splitlines() if x.strip()),'')
            if line:return line[:240]
        except (OSError,subprocess.SubprocessError):pass
    return 'version unavailable'

def _add(found:Dict[str,Toolchain],family:str,role:str,name:str,path:str,source:str)->None:
...
def discover()->List[Toolchain]:
    ...
    for family,role,name in specs:
        ...
        if p:_add(found,family,role,name,p,'PATH')
    env=os.environ
    for var in ('VCINSTALLDIR','VCToolsInstallDir','VSINSTALLDIR','LLVMInstallDir','NASM_PREFIX','MINGW_HOME','MINGW64_HOME','CODEBLOCKS'):
        ...
        for base in (root,root/'bin',root/'bin'/'Hostx64'/'x64'):
            for family,role,name in specs:_add(found,family,role,name,str(base/name),'environment:'+var)
    if os.name=='nt':
        try:
            import winreg
                ...
                try:
                    with winreg.OpenKey(hive,keypath) as key:
                        for i in range(winreg.QueryInfoKey(key)[1]):
                            version,value,_=winreg.EnumValue(key,i);base=Path(str(value))/'bin'/'Hostx64'/'x64'
                            for family,role,name in specs:
                                if family in ('MSVC','MASM'):_add(found,family,role,name,str(base/name),'registry:'+keypath+':'+version)
                except OSError:pass
