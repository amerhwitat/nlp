...
def detect_build_systems(root: Path) -> list[str]:
    checks=[('cmake',root/'CMakeLists.txt'),('make',root/'Makefile'),('make',root/'makefile'),('meson',root/'meson.build'),('cargo',root/'Cargo.toml'),('npm',root/'package.json'),('maven',root/'pom.xml'),('gradle',root/'build.gradle'),('gradle',root/'build.gradle.kts'),('dotnet',root/'*.sln'),('dotnet',root/'*.csproj'),('autotools',root/'configure.ac')]
    found=[]
...
def command_for(system:str,root:Path,build_dir:Path,compiler:str='auto')->list[str]|None:
    if system=='cmake' and shutil.which('cmake'):
        ...
        if compiler in ('msvc','cl') and shutil.which('cmake'): return ['cmake','-S',str(root),'-B',str(build_dir),'-G','Visual Studio 17 2022','-A','x64']
