...
def assemble_bios(source: Path, output: Path, nasm: str = "nasm") -> BootTestResult:
    ...
    try:
        ...
        return BootTestResult("bios","failed",f"{type(exc).__name__}: {exc}")
...
def qemu_bios_test(image: Path, timeout: int = 10, qemu: str = "qemu-system-x86_64") -> BootTestResult:
    ...
    try:
        ...
        # A boot menu that stays alive is not a crash; timeout is evidence that the VM remained running.
        return BootTestResult("bios","emulated","QEMU remained running until test timeout; inspect console evidence",command)
    except Exception as exc:
        return BootTestResult("bios","failed",f"{type(exc).__name__}: {exc}",command)
