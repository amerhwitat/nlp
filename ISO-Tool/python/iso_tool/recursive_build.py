"""Recursive repository inventory, dependency resolution, build/link orchestration and toolchain selection."""
...
class RecursiveReport:
    root:str;sources:List[SourceRecord];manifests:List[ManifestRecord];artifacts:List[BuildArtifact];skipped:List[str];errors:List[str];external_references:int=0;unresolved_references:List[str]=field(default_factory=list)
...
def acquire_repository(source:str,destination:Optional[Path]=None)->Tuple[Path,Optional[Path]]:
    ...
    else:temp=Path(tempfile.mkdtemp(prefix='iso-tool-repo-'));checkout,cleanup=temp/'repository',temp
...
def _native_direct_build(root:Path,sources:List[SourceRecord],out:Path,log:Path,cpp_compiler:Optional[str]=None,linker:Optional[str]=None,assembler:Optional[str]=None)->Tuple[List[BuildArtifact],List[str]]:
    native=[s for s in sources if s.language in {'c','cpp'}];asm=[s for s in sources if s.language=='asm']
    artifacts=[];errors=[];objdir=out/'objects';objdir.mkdir(parents=True,exist_ok=True)
    compiler=cpp_compiler or _tool('c++') or _tool('g++') or _tool('clang++');ccompiler=_tool('cc') or _tool('gcc') or _tool('clang')
    if native and not compiler and not ccompiler:errors.append('No C/C++ compiler found; native jobs remain planned.')
    objects=[]
    for record in native:
        cc=ccompiler if record.language=='c' else compiler
        if not cc:errors.append('No compiler for '+record.path);continue
        src=root/record.path;obj=objdir/(record.path.replace('/','__')+'.o');cmd=[cc,'-c','-O2','-std=c11' if record.language=='c' else '-std=c++17',str(src),'-o',str(obj)];rc=_run(cmd,root,log)
    ...
    if asm:
        chosen=assembler or _tool('nasm') or _tool('ml64') or _tool('ml') or _tool('as')
        if not chosen:errors.append('Assembly sources detected but no assembler was found.')
        else:
            name=Path(chosen).name.lower()
            for record in asm:
                src=root/record.path;obj=objdir/(record.path.replace('/','__')+'.o')
                if 'nasm' in name:cmd=[chosen,'-f','win64' if os.name=='nt' else 'elf64',str(src),'-o',str(obj)]
                elif name in ('ml.exe','ml64.exe'):cmd=[chosen,'/c','/Fo'+str(obj),str(src)]
                else:cmd=[chosen,'-c',str(src),'-o',str(obj)]
                rc=_run(cmd,root,log);artifacts.append(BuildArtifact(str(obj.relative_to(out)),'assembly-object',cmd,rc,'built' if rc==0 else 'failed'))
                if rc==0:objects.append(obj)
                else:errors.append('Assembly failed: '+record.path)
    entries=[s for s in native if s.entry_point]
    if len(entries)==1 and objects and (compiler or ccompiler):
        driver=linker or compiler or ccompiler;lname=Path(driver).name.lower()
        if lname in ('link.exe','lld-link.exe'):
            exe=out/'recursive-native.exe';cmd=[driver]+[str(p) for p in objects]+(['/OUT:'+str(exe)] if lname=='link.exe' else ['/OUT:'+str(exe)])
        else:
            exe=out/('recursive-native.exe' if os.name=='nt' else 'recursive-native');cmd=[driver]+[str(p) for p in objects]+['-o',str(exe)]
            if os.name=='nt':
    ...
    elif len(entries)>1:errors.append('Multiple native entry points detected; targets remain separate and are not force-linked together.')
...
def build_repository(root:Path,output:Optional[Path]=None,execute:bool=False,resolve_dependencies:bool=True,cpp_compiler:Optional[str]=None,linker:Optional[str]=None,assembler:Optional[str]=None)->RecursiveReport:
    ...
    (out/'toolchain-selection.json').write_text(json.dumps({'cpp_compiler':cpp_compiler or 'auto','linker':linker or 'auto','assembler':assembler or 'auto'},indent=2),encoding='utf-8')
    if not execute:return RecursiveReport(str(root),sources,manifests,[BuildArtifact(m.path,'build-manifest',status='planned') for m in manifests],skipped,errors,len(graph.references),graph.unresolved)
    log=out/'recursive-build.log';handlers={'cmake':['cmake','--build','build','--config','Release'],'make':['make'],'meson':['meson','compile','-C','build'],'cargo':['cargo','build','--release'],'go':['go','build','./...'],'maven':['mvn','-B','package'],'gradle':['gradle','build'],'msbuild':['msbuild','/m','/p:Configuration=Release'],'dotnet':['dotnet','build','-c','Release'],'node':['npm','run','build']}
    ...
    direct,direct_errors=_native_direct_build(root,sources,out,log,cpp_compiler,linker,assembler);artifacts.extend(direct);errors.extend(direct_errors)
    ...
    report=RecursiveReport(str(root),sources,manifests,artifacts,skipped,errors,len(graph.references),graph.unresolved_references if hasattr(graph,'unresolved_references') else graph.unresolved)
...
def main(argv:Optional[Sequence[str]]=None)->int:
    parser=argparse.ArgumentParser(description='Recursively inventory, resolve dependencies, build and link a GitHub/local repository');parser.add_argument('source');parser.add_argument('--output');parser.add_argument('--execute',action='store_true');parser.add_argument('--no-resolve-dependencies',action='store_true');parser.add_argument('--cpp-compiler');parser.add_argument('--linker');parser.add_argument('--assembler');args=parser.parse_args(argv)
    root,cleanup=acquire_repository(args.source,Path(args.output) if args.output else None)
    try:
        report=build_repository(root,Path(args.output)/'artifacts' if args.output and cleanup is None else None,args.execute,not args.no_resolve_dependencies,args.cpp_compiler,args.linker,args.assembler)
