...
@app.get('/api/health')
def health():
    return {'ok': True, 'database': str(DB), 'unicode_range': 'U+10A80-U+10A9F', 'api_version': '1.4.0', 'ocr': ['auto', 'kraken', 'tesseract', 'quality-only'], 'geometry': ['rtl','ltr','ttb','btt','spiral','reverse','skewed','weathered'], 'translation': ['literal','meaning','interlinear','scholarly'], 'chatbot': chatbot_health()}
...
def ocr_geometry(item: GeometryIn):
    h = analyze_geometry(item.width, item.height, edge_density=item.edge_density, contrast=item.contrast, blur=item.blur, orientation_hint=item.orientation_hint)
    return {'hypothesis': h.__dict__, 'engine_options': build_engine_options(h)}

@app.post('/api/translation/proof')
def translation_proof(item: TranslationIn):
    result = prepare_translation(item.source_text, item.source_language, item.target_language, item.mode, output=item.output, confidence=item.confidence, proof_notes=item.proof_notes, alternatives=item.alternatives, provenance=item.provenance)
    return proof_translation(result)

@app.post('/api/chat')
def chat(item: ChatIn):
    bot = ResearchChatbot()
    response = bot.generate(item.messages, item.context)
    ...
    with con() as c:
        for row in reader:
            ...
            c.execute('INSERT OR IGNORE INTO objects(external_id,title,script,site,image_url,description,rights,provenance) VALUES(?,?,?,?,?,?,?,?)',
                      (row.get('Record ID') or row.get('record_id'), title, row.get('script') or 'Old North Arabian', row.get('site'), row.get('image') or row.get('image_url'), row.get('description'), row.get('rights'), row.get('provenance'))); count += 1
...
async def pdf_import(file: UploadFile = File(...), source_id: int | None = None):
    ...
    with con() as c:
        cur = c.execute('INSERT INTO pdf_imports(source_id,filename,sha256,page_count,status,extracted_chars,warnings_json) VALUES(?,?,?,?,?,?,?)', (source_id, result.filename, result.sha256, result.page_count, 'completed', result.total_chars, json.dumps(result.warnings, ensure_ascii=False)))
        import_id = cur.lastrowid
    return {'id': import_id, 'filename': result.filename, 'sha256': result.sha256, 'page_count': result.page_count, 'total_chars': result.total_chars, 'warnings': result.warnings, 'pages': [{'page_number': p.page_number, 'text': p.text, 'char_count': p.char_count} for p in result.pages]}
