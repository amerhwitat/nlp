...
DB=Path(os.getenv('THAMUDIC_PLATFORM_DB',ROOT/'data'/'thamudic_platform.sqlite')); DB.parent.mkdir(parents=True,exist_ok=True)
app=FastAPI(title='Thamudic Cross-Language Epigraphy API',version='1.5.0')
...
class SpeechIn(BaseModel):
    text:str; language:str; voice_profile:str='default'; phonemes:list[str]=[]; model_type:str='unconfigured'; reconstructed:bool=False; confidence:float=0.0
...
def bootstrap():
    with con() as c:
        c.executescript((ROOT/'database/schema.sql').read_text(encoding='utf-8')); c.executescript((ROOT/'database/views.sql').read_text(encoding='utf-8'))
        for name in ('002_pdf_translation_kpi.sql','003_ocr_jobs.sql','004_orientation_translation_chat.sql'): c.executescript((ROOT/'database/migrations'/name).read_text(encoding='utf-8'))
@app.on_event('startup')
def startup(): bootstrap()
@app.get('/api/health')
def health(): return {'ok':True,'database':str(DB),'api_version':'1.5.0','ocr':['auto','kraken','tesseract','quality-only'],'geometry':['rtl','ltr','ttb','btt','spiral','reverse','skewed','weathered'],'translation':['literal','meaning','interlinear','scholarly'],'source_target_languages':['ancient','zh','ja','modern-bcp47'],'speech':['proof','phoneme','reconstructed-label'],'chatbot':chatbot_health()}
@app.get('/api/objects')
...
def create_object(item:ObjectIn):
    with con() as c: values=item.model_dump(); cur=c.execute('INSERT INTO objects(external_id,title,script,site,image_url,description,rights,provenance) VALUES(?,?,?,?,?,?,?,?)',tuple(values.values())); return {'id':cur.lastrowid,**values}
...
def create_reading(item:ReadingIn):
    ...
    with con() as c: values=item.model_dump(); cur=c.execute('INSERT INTO readings(object_id,reading_type,transliteration,arabic_interpretation,english_interpretation,reviewer,confidence,status) VALUES(?,?,?,?,?,?,?,?)',tuple(values.values())); return {'id':cur.lastrowid,**values}
...
@app.post('/api/speech/proof')
def speech_proof(item:SpeechIn): return prove_for_speech(item.text,item.language,voice_profile=item.voice_profile,phonemes=item.phonemes,model_type=item.model_type,reconstructed=item.reconstructed,confidence=item.confidence)
@app.post('/api/chat')
def chat(item:ChatIn): return ResearchChatbot().generate(item.messages,item.context).__dict__
...
def export_objects():
    ...
    out=io.StringIO(); writer=csv.DictWriter(out,fieldnames=rows[0].keys() if rows else ['id','title']); writer.writeheader(); [writer.writerow(dict(r)) for r in rows]
