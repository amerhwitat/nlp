...
# ---------- Safety ----------
SUSPICIOUS_KEYWORDS = {'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight'}

def looks_sensitive(df: pd.DataFrame) -> bool:
    cols = ' '.join(map(str, df.columns)).lower()
    if any(k in cols for k in SUSPICIOUS_KEYWORDS):
        return True
    sample = df.head(20).astype(str).apply(lambda s: ' '.join(s), axis=1).str.lower().str.cat(sep=' ')
...
class App:
    def __init__(self, root):
        ...
        ttk.Label(top, text="Optional text file (lines used as synthetic seeds):").grid(row=2, column=0, sticky='w', pady=(8,0))
    ...
    def run_pipeline(self):
        ...
        # Convert last column to numeric features (deterministic hashing for strings)
        ...
        X_raw = to_numeric_series(df[last_col])   # inputs are last column
        y_raw = df[first_col].astype(str)         # targets are first column (strings or numbers)

        # Decide whether target is categorical strings or numeric
        numeric_attempt = pd.to_numeric(y_raw, errors='coerce')
        non_numeric_ratio = (numeric_attempt.isna().sum()) / len(y_raw)
        is_categorical = non_numeric_ratio > 0.5

        # Optional: read txt seeds (used only to augment inputs safely)
        ...
        if extra_lines:
            extra_X = np.array([float(abs(hash(ln)) % 10_000) / 100.0 for ln in extra_lines]).reshape(-1,1)
