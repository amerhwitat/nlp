...
if (-f 'web/package.json') { my $npm = $^O eq 'MSWin32' ? 'npm.cmd' : 'npm'; run_step('Web install', $npm, (-f 'web/package-lock.json' ? 'ci' : 'install')) if chdir('web'); chdir($root); run_step('Web build',$npm,'run','build'); }
