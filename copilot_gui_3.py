# -*- coding: utf-8 -*-
"""
Created on Tue Jan 13 18:58:24 2026

@author: PC1
"""

"""
Safe Address ↔ Pubkey ↔ Local Private Key Relation Inspector

- Loads burn-addresses-btc.txt (one address per line) from current folder.
- Fetches transactions (Blockstream API) and extracts revealed public keys.
- Lets you load a local file of private keys (hex or WIF) for comparison.
- Visualizes relations and exports CSV summary.
- DOES NOT search the internet for private keys or attempt to discover keys that are not provided locally.
"""
...
def extract_pubkeys_from_input(input_obj):
    """
    Extract public keys from a tx input (scriptSig or witness).
    Returns list of hex pubkeys found.
    """
    found = []
...
class RelationInspector(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Address ↔ Pubkey ↔ Local Private Key Inspector")
        ...
        self.local_privkeys = {}  # hex -> {wif, compressed, derived_pub}
    ...
    def create_widgets(self):
        ...
        ttk.Button(frm, text="Analyze addresses", command=self.run_analysis).grid(column=1, row=0, sticky=tk.W)
        ttk.Button(frm, text="Load local private keys", command=self.load_local_privkeys).grid(column=2, row=0, sticky=tk.W)
        ttk.Button(frm, text="Export CSV", command=self.export_csv).grid(column=3, row=0, sticky=tk.W)
        ...
        self.log_box = scrolledtext.ScrolledText(frm, width=140, height=22)
        self.log_box.grid(column=0, row=4, columnspan=6, pady=(0,8))

        # Security note
        note = ("Security note: This tool is read-only for public blockchain data. "
                "Private keys loaded are kept local and not transmitted. "
                "This tool will not attempt to discover private keys on the internet.")
        ttk.Label(frm, text=note, foreground="red", wraplength=1000).grid(column=0, row=5, columnspan=6, pady=6, sticky=tk.W)
    ...
    def run_analysis(self):
        ...
        self.log("Starting analysis (read-only on-chain fetch)...")
        try:
            ...
            self.log(f"Analysis complete. Addresses: {len(self.addr_list)}; revealed pubkeys: {len(pubkey_to_addresses)}")
