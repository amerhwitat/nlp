...
class App:
    ...
    def run(self):
        ...
        if not txt or not os.path.exists(txt):
            self.queue.put(("log", "No TXT provided, skipping hex generation"))
            return
        ...
        def to_hex64(s):
            feats = np.array(string_features(s)).reshape(1,-1)
            pred = np.mean([m.forward(feats)[2] for m in models], axis=0).flatten()
            ...
            return ''.join(rng.choice(hexchars, 64))
        ...
        self.queue.put(("out", ''.join(hex_values)))
        self.queue.put(("log", f"Generated {len(hex_values)} synthetic 64-hex values → {out_path}"))(("log", f"Epoch {e}: loss={l:.6f}"))
        self.queue.put(("prog", int(100*e/self.epochs.get())))
        self.queue.put(("plot", losses.copy()))
        time.sleep(0.01)
